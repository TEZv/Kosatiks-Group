const dict = {
  ua: {
    ...
    title: "Три ролі. Один простір хабів.",
    subtitle:
    ...
    foot:
      "Не все тут продається напряму. Частина хабів — клієнтські, частина — власні системи, частина — проєктні або дослідницькі. Саме тому в кожного хаба свій маршрут переходу.",
    ...
    socialTitle: "Зовнішні переходи й публічна присутність.",
    socialSubtitle:
      "Унизу можна тримати агрегатори посилань, профілі, GitHub-маршрути й зовнішні сайти. Частина кнопок уже жива, частина поки в draft-режимі.",
    placeholder: "Під цей фільтр поки немає карток.",
    modeSummary: {
      all: "Усі формати",
      hire: "Клієнтське / для замовлення",
      self: "Власний внутрішній шар",
      project: "Активні проєкти та публічні ініціативи",
    },
    overviewIntro: "Що це",
  ...
  en: {
    ...
    foot:
      "Not every hub is a direct service offer. Some are client-facing, some are internal systems, and some are project or research layers. That is why each hub has its own route.",
    ...
    btnOpen: "Open window",
    socialKicker: "social + routing",
    socialTitle: "External routing and public presence.",
    socialSubtitle:
      "Use this footer layer for link aggregators, public profiles, GitHub routes, and external sites. Some buttons are live already; some are intentionally left in draft mode.",
    placeholder: "No cards match this filter yet.",
    modeSummary: {
      all: "All formats",
      hire: "Client-facing / commissionable",
      self: "Internal self layer",
      project: "Active projects and public initiatives",
    },
...
const socialLinks = [
  { label: { ua: "GitHub / TEZv", en: "GitHub / TEZv" }, hint: { ua: "живо", en: "live" }, url: urls.githubProfile },
  { label: { ua: "K-RnD Lab", en: "K-RnD Lab" }, hint: { ua: "research org", en: "research org" }, url: urls.krndOrg },
  { label: { ua: "LinkedIn", en: "LinkedIn" }, hint: { ua: "додати", en: "add" }, draft: true },
  { label: { ua: "Linktree", en: "Linktree" }, hint: { ua: "додати", en: "add" }, draft: true },
  { label: { ua: "Bento", en: "Bento" }, hint: { ua: "додати", en: "add" }, draft: true },
  { label: { ua: "AllMyLinks", en: "AllMyLinks" }, hint: { ua: "додати", en: "add" }, draft: true },
  { label: { ua: "Instagram / Threads", en: "Instagram / Threads" }, hint: { ua: "додати", en: "add" }, draft: true },
];
...
function getFilteredHubs() {
  return HUBS.filter((hub) => hub.role === state.facet && (state.mode === "all" || hub.modes.includes(state.mode)));
}
...
function renderSocials() {
  refs.socialRow.innerHTML = socialLinks.map((item) => {
    ...
    if (item.draft || !item.url) return `<span class="socialChip is-draft"><span>${label}</span><small>${hint}</small></span>`;
    return `<a class="socialChip" href="${escapeHtml(item.url)}" target="_blank" rel="noreferrer"><span>${label}</span><small>${hint}</small></a>`;
  }).join("");
}
